...
const PincodeDropdown: React.FC<PincodeDropdownProps> = ({
  ...
  const { selectPincode, setSelectPincode, selectCitieName, selectCities } = useLocation()
     ...
     const getPincodes = async () => {
       try {
         const response = await apiRequest(
           "GET",
           `/cities/${selectCities}/pin-codes`
         );
         if(response?.status == 200 && response.data) {
          setPincodes((response.data as {data: string[]}).data)
         }
       } catch (error) {}
     };
     if (selectCities) {
  ...
  return (
    <div
      className={`w-full mx-auto border-[#dfdfe6] border rounded p-1 shadow-lg transition ${
        isBlink ? "animate-shadow-blink" : ""
      }`}
    >
      {/* Country & Pincode Selection */}
      <div className="flex gap-5">
        <div className="w-fit text-[14px] bg-[#d1d1d1] px-6 py-3">
          {selectCitieName}
        </div>
        <Select
          options={pincodes}
